...
7. **On-chain** — follow **[btc-esplora-verify](../btc-esplora-verify/SKILL.md)** (curl/jq recipes, fingerprint checklist, hop rules). Prefer `mempool.bitaroo.net` or `mempool.emzy.de` for bulk scans; do not stampede `mempool.space` / Blockstream. Pace ≥400ms between host calls.
8. **Cash-outs / labels** — when vaults empty or hops hit exchanges/bridges, follow **[cashout-labeling](../cashout-labeling/SKILL.md)** (holding vs label-only, Arkham, `KNOWN_ADDRESS_LABELS`).
