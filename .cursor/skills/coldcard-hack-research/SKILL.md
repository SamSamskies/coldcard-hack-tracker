...
3. **X/Twitter** — prefer the **X MCP** (`xapi` via `@xdevplatform/xurl` → `https://api.x.com/mcp`). **Do not call `search_posts_all`** — full-archive search needs App-Only Bearer + pay-per-use; our MCP session is user OAuth and that tool 403s. Use read tools only: `get_users_by_username` → `get_users_posts` on known reporters (see [reference.md](reference.md)), then `get_posts_by_id` / `get_posts_by_ids` for threads. Optional recent (7-day) keyword search via CLI: `npx -y @xdevplatform/xurl search 'QUERY' -n 20` — not via MCP. Pace calls — quotas burn fast. Keep canonical `x.com` URLs in `sourceUrl`. For Galaxy chart **media/OCR**, MCP text is often enough; if you need `pbs.twimg.com/...name=orig` image URLs, fall back to **[xcancel.com](https://xcancel.com/)** (rewrite `https://x.com/...` → `https://xcancel.com/...`). If MCP auth fails or is rate-limited, try xcancel, then ask the user for text/screenshot. Never guess totals from a broken fetch. Do not use bookmark/write tools for research.
